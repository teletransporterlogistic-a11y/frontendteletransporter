// src/pages/incidencias/DanosPage.tsx
...
export default function DanosPage() {
  const [data, setData] = useState<IncidenciaDano[]>([]);
  ...
  const cargar = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Error al cargar incidencias de daños");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Incidencias de Daños</h1>
            ...
            <tbody>
              {data.map((i) => (
                <tr key={i.id}>
                  ...
                  <td className="p-2">{i.envio?.guia ?? "N/A"}</td>
                  <td className="p-2">{i.descripcion}</td>
                  <td className="p-2">{i.operador?.nombre ?? "N/A"}</td>
