...
export default function ReciboPreview() {
  ...
  // 🔥 Datos recibidos desde envíos o entregas
  const envio = location.state?.envio;
  const clienteEntrega = location.state?.cliente; // cliente seleccionado en ENTREGAS
  ...
  // 🔥 Cliente final (envíos normales o entregas)
  const cliente =
    clienteEntrega || envio.cliente || { nombre: "CLIENTE", rfc: "N/A" };

  const total = envio.total ?? envio.costoTotal ?? envio.costo ?? 0;
