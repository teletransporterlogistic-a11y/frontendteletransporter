...
export default function EntregaProceso() {
  ...
  // Si venimos de seleccionar cliente en ClientesList
  useEffect(() => {
  ...
  async function entregarPaquete() {
    ...
    // Si es FXC → abrir pantalla de pago
    ...
    // Si es PAG → entregar directo
    try {
      await api.post(`/envios/${envio.id}/entregar`, {
        clienteId: clienteSeleccionado.id,
      });

      // 🔥 Recargar envío actualizado DESPUÉS de entregar
      const actualizado = await api.get(`/envios/${envio.id}/completo`);

      // 🔥 Navegar al recibo de ENTREGA (sin costos)
      navigate(`/envios/recibo-entrega`, {
        state: { envio: actualizado.data, cliente: clienteSeleccionado },
      });
    } catch (err) {
  ...
  if (cargando) return <div className="loading">Cargando datos del envío...</div>;
  ...
  return (
    <div className="entrega-proceso-wrapper">
      ...
      <div className="card datos-envio">
        ...
        <p><strong>Ciudad destino:</strong> {envio.destinatarioCiudad}, {envio.destinatarioEstado}</p>
        <p><strong>Peso:</strong> {envio.peso} kg</p>
        <p><strong>Tipo de pago:</strong> {envio.pago_destino ? "FXC (Paga el destinatario)" : "PAG (Pagado)"}</p>
      </div>
