...
export default function RastreoEnvio() {
  ...
  // Cargar envío por ID
  // ============================
  async function cargarEnvioPorId(id) {
    try {
  ...
  // ============================
  async function cargarEnvioPorGuia(guia) {
    try {
  ...
  useEffect(() => {
    ...
    // Escuchar actualizaciones reales
    socketRef.current.on("rastreo:update", (data) => {
    ...
    return () => {
      socketRef.current.off("rastreo:update");
    };
  }, [envio]);
  ...
  return (
    <div className="rastreo-container">
      ...
      {id && (
        <button className="btn-secondary mb-20" onClick={() => navigate(`/envios/${id}`)}>
          ← Regresar al detalle
        </button>
      )}
      ...
      {envio && (
        <div className="info-card">
          ...
          <p><strong>Última actualización:</strong> {ultimaPos ? new Date(ultimaPos.fecha).toLocaleString() : "Sin datos"}</p>
        </div>
      )}
